# NEWTUBE Security Module
# Creates security groups and IAM policies for the application
...
resource "aws_security_group" "ecs" {
  name_prefix = "${var.name_prefix}-${var.environment}-ecs-"
  vpc_id      = var.vpc_id
  ...
  ingress {
    description = "ECS tasks communication"
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
    self        = true
...
# Lambda Security Group (for database initialization)
...
# Redis Security Group (for caching)
...
resource "aws_wafv2_web_acl" "main" {
  ...
  rule {
    ...
    visibility_config {
      ...
      metric_name                = "${var.name_prefix}-${var.environment}-rate-limit"
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "${var.name_prefix}-${var.environment}-common-rules"
      sampled_requests_enabled   = true
    }
  }
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "${var.name_prefix}-${var.environment}-bad-inputs"
      sampled_requests_enabled   = true
    }
  }
